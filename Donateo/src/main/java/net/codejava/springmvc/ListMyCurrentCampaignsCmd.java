...
public class ListMyCurrentCampaignsCmd implements Command {
		...
		try {
			...
			if (con != null) {
				...
				CallableStatement cStmt = con
						.prepareCall("{call listMyCurCampaigns(?,?)}");
				...
				try {
					...
					ArrayList<Campaign> user_current_campaigns = new ArrayList<Campaign>();
					...
					while (rs.next()) {
						...
						System.out.println("project name: "
								+ c.getProject_name());
						user_current_campaigns.add(c);

					}

					htblOutputParams.put("listMyCurrentCampaigns", user_current_campaigns);
