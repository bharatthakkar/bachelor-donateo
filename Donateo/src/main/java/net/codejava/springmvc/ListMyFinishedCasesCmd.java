...
public class ListMyFinishedCasesCmd implements Command {
		...
		try {
			...
			if (con != null) {
				...
				CallableStatement cStmt = con
						.prepareCall("{call listMyFinishedCases(?,?)}");
				...
				try {
					...
					ArrayList<Cases> user_finished_cases = new ArrayList<Cases>();
					...
					while (rs.next()) {
						...
						System.out.println("project name: "
								+ c.getProject_name());
						user_finished_cases.add(c);

					}

					htblOutputParams.put("listMyFinishedCases", user_finished_cases);
