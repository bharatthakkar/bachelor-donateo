...
public class ListMyFinishedCrowdfundingsCmd implements Command {
		...
		try {
			...
			if (con != null) {
				...
				CallableStatement cStmt = con
						.prepareCall("{call listMyFinishedCrowdFundingProjects(?,?)}");
				...
				try {
					...
					ArrayList<Crowdfundings> user_finished_crowdfundings = new ArrayList<Crowdfundings>();
					...
					while (rs.next()) {
						...
						System.out.println("project name: "
								+ c.getProject_name());
						user_finished_crowdfundings.add(c);

					}

					htblOutputParams.put("listMyFinishedCrowdfundings", user_finished_crowdfundings);
